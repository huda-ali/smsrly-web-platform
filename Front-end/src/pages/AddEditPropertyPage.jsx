...
// The `coordinates` prop is the single source of truth — the parent controls the pin.
// Future: backend/AI can push new coordinates into `coordinates` without any
// changes to this component's interface.
function PropertyMap({ coordinates, onCoordinatesChange }) {
  ...
  useEffect(() => {
    // Dynamically import Leaflet (avoids SSR issues with Vite)
    let cancelled = false;
    ...
    async function initMap() {
      ...
      const map = L.map(containerRef.current, {
        center: coordinates || [30.0444, 31.2357], // Cairo default
      ...
      if (coordinates) {
        markerRef.current = L.marker(coordinates, { draggable: true }).addTo(
          map,
        );
  ...
  // When coordinates change from outside (location select), fly the map there
  useEffect(() => {
    if (!leafletRef.current || !coordinates) return;
    const { map, L } = leafletRef.current;
    map.flyTo(coordinates, 12, { duration: 1 });
    if (markerRef.current) {
      markerRef.current.setLatLng(coordinates);
    } else {
      markerRef.current = L.marker(coordinates, { draggable: true }).addTo(map);
      markerRef.current.on("dragend", () => {
        const pos = markerRef.current.getLatLng();
        onCoordinatesChange?.([pos.lat, pos.lng]);
      });
...
export default function AddEditPropertyPage() {
  ...
  const [draftStatus, setDraftStatus] = useState(""); // 'saved' | 'error' | ''
  const [draftLoaded, setDraftLoaded] = useState(false);
  const [currentView, setCurrentView] = useState("details"); // 'details' | 'review'
  ...
  // ── Step completion + dynamically active step for the sidebar ──
  const isStep1Done =
  ...
  // ── Draft fields snapshot (excludes binary images) ──
  ...
  // ── On mount: load draft or existing property ──
  useEffect(() => {
    if (isEdit && existing) {
  ...
  // ── When governorate changes, reset downstream fields and snap map ──
  ...
  // ── Validate steps 1-3 and move to the Review step ──
  const goToReview = () => {
    const fields = {
      title,
      description,
    ...
    if (Object.keys(errs).length > 0) {
      ...
      // Scroll to first error (wait a tick in case we just switched views)
      setTimeout(() => {
  ...
  // ── Publish / Save changes: persists to the real property store ──
  const handlePublish = async () => {
    ...
    // Simulate network latency; swap for a real API call later without
    // changing the callers below.
