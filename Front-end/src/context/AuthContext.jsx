...
function seedAllUsers() {
  const longAgo = Date.now() - 120 * 24 * 60 * 60 * 1000; // 120 days ago
...
export function AuthProvider({ children }) {
  ...
  // ── Notifications: every entry is attributed to a real authenticated actor ──
  // `actor` must be a signed-in user object (never null/guest) — notifications
  // are only ever generated as a side effect of something an authenticated
  // user actually did (signing up, publishing a listing, an admin action…).
  const addNotification = useCallback(
    (
      actor,
      { targetRole = null, targetUserId = null, type = "general", content },
    ) => {
      if (!actor || !actor.id) return; // no real authenticated user behind this event — don't record it
      const entry = {
      ...
      setNotifications((prev) => {
        const updated = [entry, ...prev].slice(0, 200); // keep it bounded
  ...
  // ── User directory (persisted across sessions, drives Admin User Management) ──
  ...
  // Simulated OAuth: in a real app this would redirect to the provider and
  // come back with a verified email. Here we collect the email directly and
  // either sign the person into their existing account or create a new one.
  const socialAuth = useCallback(
    (provider, email, name) => {
      ...
      if (existing) {
        ...
        localStorage.setItem("smsrly_user", JSON.stringify(safeUser));
        touchUserActivity(safeUser.id);
        upsertAllUser({ id: safeUser.id, provider });
        return { success: true, user: safeUser };
      }
    ...
    [touchUserActivity, upsertAllUser, addNotification],
  );
